/**
 * SVG Icon Sprite — Школа финансовой культуры
 * Загружается синхронно в начале <body>, делая все иконки доступными сразу.
 * Использование: <svg class="icon"><use href="#ic-имя"></use></svg>
 */
(function () {
  'use strict';
  var sprite = [
    '<svg xmlns="http://www.w3.org/2000/svg" style="display:none" aria-hidden="true">',
    '<defs>',

    /* ic-chart-bar — статистика, обзор */
    '<symbol id="ic-chart-bar" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<rect x="3" y="3" width="18" height="18" rx="2"/>',
    '<path d="M7 16V11M12 16V8M17 16V12"/>',
    '</symbol>',

    /* ic-chart-line — прогресс, тренды */
    '<symbol id="ic-chart-line" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/>',
    '<polyline points="17 6 23 6 23 12"/>',
    '</symbol>',

    /* ic-users — пользователи */
    '<symbol id="ic-users" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/>',
    '<circle cx="9" cy="7" r="4"/>',
    '<path d="M23 21v-2a4 4 0 0 0-3-3.87"/>',
    '<path d="M16 3.13a4 4 0 0 1 0 7.75"/>',
    '</symbol>',

    /* ic-calendar — мероприятия, даты */
    '<symbol id="ic-calendar" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<rect x="3" y="4" width="18" height="18" rx="2" ry="2"/>',
    '<line x1="16" y1="2" x2="16" y2="6"/>',
    '<line x1="8" y1="2" x2="8" y2="6"/>',
    '<line x1="3" y1="10" x2="21" y2="10"/>',
    '</symbol>',

    /* ic-scale — жюри, весы */
    '<symbol id="ic-scale" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<line x1="12" y1="3" x2="12" y2="21"/>',
    '<path d="M5 21h14"/>',
    '<path d="M5 7l3 8a2 2 0 0 1-6 0z"/>',
    '<path d="M19 7l3 8a2 2 0 0 1-6 0z"/>',
    '</symbol>',

    /* ic-document — контент, файлы с текстом */
    '<symbol id="ic-document" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>',
    '<polyline points="14 2 14 8 20 8"/>',
    '<line x1="16" y1="13" x2="8" y2="13"/>',
    '<line x1="16" y1="17" x2="8" y2="17"/>',
    '</symbol>',

    /* ic-settings — настройки, профиль */
    '<symbol id="ic-settings" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<circle cx="12" cy="12" r="3"/>',
    '<path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/>',
    '</symbol>',

    /* ic-trophy — награды, достижения */
    '<symbol id="ic-trophy" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<polyline points="8 21 12 21 16 21"/>',
    '<line x1="12" y1="17" x2="12" y2="21"/>',
    '<path d="M7 4H4v7a5 5 0 0 0 5 5h6a5 5 0 0 0 5-5V4h-3"/>',
    '<path d="M7 4h10"/>',
    '</symbol>',

    /* ic-upload — загрузка работ */
    '<symbol id="ic-upload" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<polyline points="16 16 12 12 8 16"/>',
    '<line x1="12" y1="12" x2="12" y2="21"/>',
    '<path d="M20.39 18.39A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.3"/>',
    '</symbol>',

    /* ic-check-circle — выполнено, завершено */
    '<symbol id="ic-check-circle" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/>',
    '<polyline points="22 4 12 14.01 9 11.01"/>',
    '</symbol>',

    /* ic-check — галочка (features-list) */
    '<symbol id="ic-check" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">',
    '<polyline points="20 6 9 17 4 12"/>',
    '</symbol>',

    /* ic-clock — дедлайны, таймер */
    '<symbol id="ic-clock" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<circle cx="12" cy="12" r="10"/>',
    '<polyline points="12 6 12 12 16 14"/>',
    '</symbol>',

    /* ic-mail — уведомления, email */
    '<symbol id="ic-mail" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>',
    '<polyline points="22,6 12,13 2,6"/>',
    '</symbol>',

    /* ic-phone — телефон */
    '<symbol id="ic-phone" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12.18 19.79 19.79 0 0 1 1.61 3.6 2 2 0 0 1 3.6 1.42h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L7.91 8.95a16 16 0 0 0 6.08 6.08l1.61-1.63a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>',
    '</symbol>',

    /* ic-chat — поддержка, сообщения */
    '<symbol id="ic-chat" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>',
    '</symbol>',

    /* ic-file — файл документа */
    '<symbol id="ic-file" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"/>',
    '<polyline points="13 2 13 9 20 9"/>',
    '</symbol>',

    /* ic-download — скачать */
    '<symbol id="ic-download" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>',
    '<polyline points="7 10 12 15 17 10"/>',
    '<line x1="12" y1="15" x2="12" y2="3"/>',
    '</symbol>',

    /* ic-eye — просмотр */
    '<symbol id="ic-eye" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>',
    '<circle cx="12" cy="12" r="3"/>',
    '</symbol>',

    /* ic-edit — редактировать */
    '<symbol id="ic-edit" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>',
    '<path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>',
    '</symbol>',

    /* ic-trash — удалить */
    '<symbol id="ic-trash" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<polyline points="3 6 5 6 21 6"/>',
    '<path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/>',
    '</symbol>',

    /* ic-close — закрыть, отменить */
    '<symbol id="ic-close" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<line x1="18" y1="6" x2="6" y2="18"/>',
    '<line x1="6" y1="6" x2="18" y2="18"/>',
    '</symbol>',

    /* ic-save — сохранить */
    '<symbol id="ic-save" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/>',
    '<polyline points="17 21 17 13 7 13 7 21"/>',
    '<polyline points="7 3 7 8 15 8"/>',
    '</symbol>',

    /* ic-clipboard — назначенные работы */
    '<symbol id="ic-clipboard" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/>',
    '<rect x="8" y="2" width="8" height="4" rx="1" ry="1"/>',
    '</symbol>',

    /* ic-pen — проверка работ */
    '<symbol id="ic-pen" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/>',
    '</symbol>',

    /* ic-medal — медаль (для наград: золото/серебро/бронза) */
    '<symbol id="ic-medal" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<circle cx="12" cy="8" r="6"/>',
    '<path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11"/>',
    '</symbol>',

    /* ic-award — сертификат, участие */
    '<symbol id="ic-award" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<circle cx="12" cy="8" r="6"/>',
    '<path d="M8.56 2.75c4.37 6.03 6.02 9.42 8.03 17.72m2.54-15.38c-3.72 4.35-8.94 5.66-16.88 5.85m19.5 1.9c-3.5-.93-6.63-.82-8.94 0-2.58.92-5.01 2.86-7.44 6.32"/>',
    '</symbol>',

    /* ic-location — место проведения */
    '<symbol id="ic-location" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/>',
    '<circle cx="12" cy="10" r="3"/>',
    '</symbol>',

    /* ic-export — экспорт отчёта */
    '<symbol id="ic-export" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>',
    '<polyline points="17 8 12 3 7 8"/>',
    '<line x1="12" y1="3" x2="12" y2="15"/>',
    '</symbol>',

    /* ic-user — одиночный пользователь/профиль */
    '<symbol id="ic-user" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">',
    '<path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>',
    '<circle cx="12" cy="7" r="4"/>',
    '</symbol>',

    '</defs>',
    '</svg>'
  ].join('');

  /* Вставляем спрайт как первый дочерний элемент <body> */
  document.body.insertAdjacentHTML('afterbegin', sprite);
}());
